@extends('layouts.app')
@section('title', 'Cash Out Records - Finance')
...
<div style="background:var(--grid-bg); min-height:calc(100vh - 62px); padding:24px;">
    <div style="max-width:1440px; margin:0 auto;">
        ...
        <div style="display:flex; gap:2px; margin-bottom:16px;">
            <a href="{{ route('finance.cash-out.index') }}" style="padding:8px 16px; background:#e2e8f0; color:#475569; font-size:12px; font-weight:600; text-transform:uppercase; text-decoration:none; border-top-left-radius:4px; border-top-right-radius:4px;">
                View Detail
            </a>
            <a href="{{ route('finance.cash-out.records-list') }}" style="padding:8px 16px; background:var(--b-pri); color:#fff; font-size:12px; font-weight:600; text-transform:uppercase; text-decoration:none; border-top-left-radius:4px; border-top-right-radius:4px;">
                Records List
            </a>
        </div>
            ...
            <div class="grid-footer">
                ...
                <div class="grid-totals">
                    <div style="text-align:right;">
                        <div style="font-size:10px; color:#64748b;">TOTAL DP</div>
                        <div id="totalDp">0.00</div>
                    </div>
                    <div style="text-align:right;">
                        <div style="font-size:10px; color:#64748b;">TOTAL SUM</div>
...
<script>
    ...
    var processedData = tableData.map(item => {
        return {
            ...
            dp: parseFloat(item.dp_amount || 0).toFixed(2),
            total: parseFloat(item.cash_amount || 0).toFixed(2),
    ...
    var table = new Tabulator("#erpGrid", {
        ...
        groupBy: false, 
        keybindings: true, // Keyboard auto-bindings supported natively
        columns: [
            {title: "CHK", field: "chk", formatter: checkFormatter, hozAlign: "center", headerHozAlign: "center", width: 40, headerSort: false, resizable: false},
            ...
            {title: "RATE", field: "rate", hozAlign: "right", formatter: moneyFormatter, width: 80},
            {title: "USER NO", field: "user", hozAlign: "left"},
            {title: "BANK", field: "bank", hozAlign: "left", width: 160},
            {title: "DP", field: "dp", hozAlign: "right", formatter: moneyFormatter, width: 120, bottomCalc: "sum", bottomCalcFormatter: moneyFormatter},
            {title: "TOTAL", field: "total", hozAlign: "right", formatter: moneyFormatter, width: 120, bottomCalc: "sum", bottomCalcFormatter: moneyFormatter},
            {title: "EMPLOYEE", field: "employee", hozAlign: "left"}
        ],
    });
    ...
    table.on("renderComplete", function(){
        ...
        if(calcData.bottom) {
            document.getElementById('totalDp').innerText = parseFloat(calcData.bottom.dp).toLocaleString('en-US', {minimumFractionDigits:2});
            document.getElementById('totalSum').innerText = parseFloat(calcData.bottom.total).toLocaleString('en-US', {minimumFractionDigits:2});
        }
    });
    ...
    document.getElementById("quickSearch").addEventListener("keyup", function(){
        ...
        table.setFilter(function(data){
            return String(data.ref).toLowerCase().includes(term) || 
                   ...
                   String(data.employee).toLowerCase().includes(term);
        });
    });

    table.on("rowDblClick", function(e, row){
        let id = row.getData().id;
        // Navigation route when detail form is fully adapted for view
        alert('Opening Detail View for Cash Out Trans ID: ' + id);
        // window.location.href = "{{ url('/finance/cash-out') }}/" + id;
    });
