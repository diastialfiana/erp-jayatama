@extends('layouts.app')
@section('title', 'Cash Out - Finance')
...
<div style="background:var(--grid-bg); min-height:calc(100vh - 62px); padding:24px;">
    <div style="max-width:1440px; margin:0 auto;">
        ...
        <form id="cashOutForm" method="POST" action="{{ route('finance.cash-out.store') }}">
            @csrf
            <input type="hidden" name="details" id="hiddenDetails">

            <!-- ERP Tab Navigation -->
            <div style="display:flex; gap:2px; margin-bottom:16px;">
                <a href="{{ route('finance.cash-out.index') }}" style="padding:8px 16px; background:var(--b-pri); color:#fff; font-size:12px; font-weight:600; text-transform:uppercase; text-decoration:none; border-top-left-radius:4px; border-top-right-radius:4px;">
                    View Detail
                </a>
                <a href="{{ route('finance.cash-out.records-list') }}" style="padding:8px 16px; background:#e2e8f0; color:#475569; font-size:12px; font-weight:600; text-transform:uppercase; text-decoration:none; border-top-left-radius:4px; border-top-right-radius:4px;">
                    Records List
                </a>
            </div>
            ...
            <div class="erp-box form-2col">
                ...
                <div>
                    ...
                    <div class="form-line">
                        <span class="form-label">Employee / User</span>
                        <select name="employee_id" class="erp-input" tabindex="4">
                            <option value="">- Select User -</option>
                            <option value="{{ auth()->id() ?? 1 }}">{{ auth()->user()->name ?? 'Administrator' }}</option>
                        </select>
                    </div>
                    <div class="form-line">
                        <span class="form-label">CA Reference</span>
                        <input type="text" name="ca_reference" class="erp-input" placeholder="Cash Advance Ref..." tabindex="5">
                    </div>
                </div>
                ...
                <div>
                    <div class="form-line">
                        ...
                        <input type="text" name="reference" class="erp-input" placeholder="AUTO ENTRY" tabindex="6">
                    </div>
                    <div class="form-line">
                        <span class="form-label">Rate</span>
                        <input type="number" name="rate" class="erp-input" value="1" step="0.01" tabindex="7">
                    </div>
                    <div class="form-line" style="margin-top:12px;">
                        <label style="display:flex; align-items:center; gap:8px; font-size:12px; font-weight:600; color:#334155; cursor:pointer;">
                            <input type="checkbox" name="is_down_payment" value="1" tabindex="8" style="width:16px; height:16px;">
                            DOWN PAYMENT
                        </label>
                    </div>
                </div>
...
<script>
    ...
    // Fetch Accounts using Cash In API mapping as requested
    ...
    function renderGrid() {
        ...
        rows.forEach((r, idx) => {
            ...
            tr.innerHTML = `
                ...
                <td><input type="text" class="cell-input field-account" data-idx="${idx}" value="${r.account_code}" list="accountList" placeholder="Code..." autocomplete="off"></td>
                <td><input type="text" class="cell-input field-name" data-idx="${idx}" value="${r.account_name}" readonly style="background:#f8fafc; color:#64748b;"></td>
    ...
    // Double enter or shortcut trick
    ...
    // Auto shortcut (Excel style ctrl+D line copy mock logic could be here, bonus stuff)
    document.addEventListener('keydown', function(e) {
        if(e.ctrlKey && e.key === 'd') {
            e.preventDefault();
            // Duplicate row logic for bonus PRO
