@extends('layouts.app')
@section('title', 'Cash Transfer Records - Finance')
...
<div style="background:var(--grid-bg); min-height:calc(100vh - 62px); padding:24px;">
    <div style="max-width:1440px; margin:0 auto;">
        ...
        <div style="display:flex; gap:2px; margin-bottom:16px;">
            <a href="{{ route('finance.cash-transfer.index') }}" style="padding:8px 16px; background:#e2e8f0; color:#475569; font-size:12px; font-weight:600; text-transform:uppercase; text-decoration:none; border-top-left-radius:4px; border-top-right-radius:4px;">
                View Detail
            </a>
            <a href="{{ route('finance.cash-transfer.records-list') }}" style="padding:8px 16px; background:var(--b-pri); color:#fff; font-size:12px; font-weight:600; text-transform:uppercase; text-decoration:none; border-top-left-radius:4px; border-top-right-radius:4px;">
                Record List
            </a>
        </div>
...
<script>
    var tableData = @json($transfers);
    
    var processedData = tableData.map(item => {
        return {
            id: item.id,
            date: item.date,
            bank: item.from_bank ? item.from_bank.code : '-',
            bank_name: item.from_bank ? item.from_bank.bank_name : '-',
            curr: item.currency,
    ...
    var table = new Tabulator("#erpGrid", {
        ...
        pagination: "local",
        paginationSize: 50,
        placeholder: "No Transfers Found",
        groupBy: false, 
        keybindings: true, // Keyboard bindings
        columns: [
            ...
            {title: "BANK", field: "bank", hozAlign: "left", width: 100},
            {title: "BANK NAME", field: "bank_name", hozAlign: "left", width: 150},
            {title: "CURR", field: "curr", hozAlign: "center", width: 80},
            {title: "RATE", field: "rate", hozAlign: "right", formatter: moneyFormatter, width: 80},
            {title: "REF", field: "ref", hozAlign: "left"},
            {title: "LINK", field: "link", hozAlign: "center", width: 60},
            {title: "USER NO", field: "user", hozAlign: "left"},
            {title: "TOTAL", field: "total", hozAlign: "right", formatter: moneyFormatter, width: 140, bottomCalc: "sum", bottomCalcFormatter: moneyFormatter},
    ...
    document.getElementById("quickSearch").addEventListener("keyup", function(){
        ...
        table.setFilter(function(data){
            return String(data.ref).toLowerCase().includes(term) || 
                   ...
                   String(data.bank_name).toLowerCase().includes(term) ||
    ...
    table.on("rowDblClick", function(e, row){
        ...
        // Navigation route when detail form mode is needed
        alert('Opening Detail View for Cash Transfer ID: ' + id);
        // window.location.href = "{{ url('/finance/cash-transfer') }}/" + id;
    });
