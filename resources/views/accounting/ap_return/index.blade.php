...
@section('content')
<div x-data="apReturnManager()" x-init="init()" x-on:ribbon-action.window="handleRibbonAction($event.detail)" style="background: white; border: 1px solid var(--hr-border); margin: 10px; display: flex; flex-direction: column; height: calc(100vh - 80px);">
    ...
    <div class="window-title-bar">
        <div style="display: flex; gap: 8px; align-items: center;">
            ...
            <span style="font-weight: 500; font-size: 0.8rem;">Supplier Return</span>
        </div>
            ...
            <div class="detail-form-area" x-show="selectedReturn">
                <div style="display: flex; gap: 20px; flex: 1;">
                    <div>
                        ...
                        <div class="form-group">
                            <div class="form-label" style="width: 80px; text-align: right;">Supplier name</div>
                            <div class="custom-combobox" style="width: 250px; position: relative;" @click.away="supplierOpen = false">
                                <input type="text" class="combo-input" x-model="selectedReturn.supplier_name" @focus="supplierOpen = true" @input="supplierOpen = true; saveCurrentChanges()" @change="saveCurrentChanges()">
                                ...
                                <div class="combo-dropdown" x-show="supplierOpen" style="display: none;">
                                    <template x-for="opt in suppliers" :key="opt">
                                        <div class="combo-option" :class="selectedReturn.supplier_name === opt ? 'selected' : ''" @click="selectedReturn.supplier_name = opt; saveCurrentChanges(); supplierOpen = false" x-text="opt"></div>
                                    </template>
                                </div>
                            </div>
                        </div>
                ...
                <div style="display: flex; align-items: flex-end; padding-right: 20px;">
                    <div style="font-size: 1.5rem; font-weight: 800; color: #475569; text-transform: lowercase;">supplier return</div>
                </div>
            </div>
                    ...
                    <tbody style="border-bottom: none;">
                        <template x-for="(item, idx) in returns" :key="item.id">
                            <tr class="row-data" @click="selectReturn(item.id)" :class="selectedReturn && selectedReturn.id === item.id ? 'selected' : ''" :style="selectedReturn && selectedReturn.id === item.id ? 'background:#e0b0ff; color:indigo;' : ''">
                                ...
                                <td x-text="item.supplier_name" style="color:#d946ef;"></td>
...
@push('scripts')
<script>
    function apReturnManager() {
        return {
            activeMainTab: 'detail',
            returns: @json($apReturns),
            selectedReturn: null,
            ...
            suppliers: ['REGIONAL OFFICE MEDAN', 'PT BINTANG KARYA', 'CV JASA SWADAYA UTAMA', 'Koperasi KITA'],
            rates: ['1', '15000', '16000'],
            ...
            createNew() {
                ...
                this.activeMainTab = 'detail';
                showToast('New supplier return created', 'success');
            },
